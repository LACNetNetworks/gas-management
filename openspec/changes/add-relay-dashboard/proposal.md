...
Va antes que `add-nonce-reordering` a proposito: sin el dashboard, validar el buffer de
reordenamiento es leer logs a mano. Corresponde a la fase **F3** de
`docs/PLAN-reorden-dashboard.md`.

## What Changes

- **`GET /dashboard/stream`**: SSE sobre el bus de eventos de `add-relay-event-bus`. Reanudacion
...
- **`GET /dashboard`**: la pagina. Se porta `node_relayer/src/dashboard-page.ts` a
  ...
  se modifica: consume el mismo vocabulario de eventos que `add-relay-event-bus` congelo.
...
- **Riesgo**: si el emisor de eventos se desincroniza del frontend portado, la pagina deja de
  pintar sin error visible. Lo cubre el contrato congelado en `add-relay-event-bus`.
- **Depende de**: `add-relay-event-bus` (bus, `bufferSize`, contrato de eventos) y de
  `add-relay-http-endpoints` (router por path).
